...
//        MLog.info("找点用时：", String.valueOf(System.currentTimeMillis() - now));
    ...
    public static Point findPointByCheckImageModel(Bitmap screenshot, CheckImageModel checkImageModel) {
        return Image.matchTemplate(screenshot, checkImageModel.image,0.9, checkImageModel.rectangle);
    }
    ...
    public static Point findPointByCheckImageModels(Bitmap screenshot, CheckImageModel[] checkImageModels) {
        ...
        for (CheckImageModel checkImageMode :
                checkImageModels) {
            p = Image.matchTemplate(screenshot,checkImageMode.image,0.9,checkImageMode.rectangle);
            if(p.isValid()){
                return p;
            }
        }
